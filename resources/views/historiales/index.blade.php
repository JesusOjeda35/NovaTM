...
<div class="container mx-auto px-4 py-8">
    ...
    <!-- SECCIÓN 2: CITAS SIN HISTORIAL (solo para profesionales - AL PRINCIPIO) -->
    @if(auth()->user()->isProfesional())
        @php
            $disponibilidadesIds = \App\Models\Disponibilidad::where('user_id', auth()->user()->id)->pluck('id')->toArray();
            
            // Obtener consultas SIN historial (donde historial_id es NULL)
    ...
    <div>
        ...
        @else
            <div class="grid gap-6">
                @foreach($historiales as $historial)
                    <div class="bg-white rounded-lg shadow-lg p-6 border-l-4 border-blue-500 hover:shadow-xl transition"
                         data-historial-id="{{ $historial->id_historial }}"
                         data-animal-name="{{ $historial->animal->nombre }}">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                            <!-- Animal -->
                            <div>
                                <h3 class="text-lg font-bold mb-3 text-gray-800">🐴 Animal</h3>
                        ...
                        <div class="mt-6 pt-6 border-t flex gap-2 flex-wrap">
                            ...
                            @if(auth()->user()->isProfesional() && $historial->user_id === auth()->user()->id)
                                ...
                                <button type="button" 
                                        ...
                                        onclick="openDeleteModal({{ $historial->id_historial }})">
                                    🗑️ Eliminar
                                </button>
                            @endif
                        </div>
            ...
            <div class="modal fade" id="deleteHistorialModal" tabindex="-1" role="dialog">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header" style="background: linear-gradient(135deg, #dc2626 0%, #991b1b 100%); color: white; border: none; border-radius: 16px 16px 0 0; padding: 28px;">
                        ...
                        <div class="modal-body" style="padding: 32px; text-align: center;">
                            <div style="font-size: 64px; color: #dc2626; margin-bottom: 16px;">
                                <i class="fas fa-exclamation-triangle"></i>
                            </div>
                            <p style="font-size: 16px; color: #6b7280; line-height: 1.6;">
                                ¿Estás seguro de que deseas eliminar el historial del animal <strong id="animalName"></strong>?<br>
                                <small style="color: #9ca3af;">Esta acción no se puede deshacer.</small>
                            </p>
                        </div>
                        <div class="modal-footer" style="border: none; padding: 20px 28px; background: #f9fafb; border-radius: 0 0 16px 16px; display: flex; gap: 12px; justify-content: flex-end;">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal" style="background: #f3f4f6; color: #14202a; border: none; padding: 12px 24px; border-radius: 10px; font-weight: 600; cursor: pointer;">
                                <i class="fas fa-times"></i> Cancelar
                            </button>
                            <form id="deleteForm" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger" style="background: #dc2626; color: white; border: none; padding: 12px 24px; border-radius: 10px; font-weight: 600; cursor: pointer;">
...
<script>
function openDeleteModal(historialId) {
    // Obtener el elemento con los datos
    const historialElement = document.querySelector(`[data-historial-id="${historialId}"]`);
    const animalName = historialElement.getAttribute('data-animal-name');
    
    // Actualizar formulario con la ruta correcta
    const deleteForm = document.getElementById('deleteForm');
    deleteForm.action = `/historiales/${historialId}`;
    
    // Actualizar nombre del animal en el modal
    document.getElementById('animalName').textContent = animalName;
    
    // Abrir modal
    $('#deleteHistorialModal').modal('show');
}
</script>
@endsection